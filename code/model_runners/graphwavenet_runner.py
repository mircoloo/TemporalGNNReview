...
class GraphWaveNetRunner(BaseModelRunner):
    ...
    def train(self, train_dataset, 
        ...
        for epoch in range(1, num_epochs+1):
            ...
            for batch in train_loader:
                ...
                batch_acc += ((torch.round(torch.sigmoid(predict))).detach().int().cpu() == real.detach().int().cpu()).float().mean().item()
            ...
            if val_dataset and (epoch % 5 == 0):
                ...
                with torch.no_grad():
                    for batch in val_loader:          
                        ...
                        output_for_loss = output[:, :, :, -1]  # Take last timestep
                        predict = output_for_loss.squeeze(1)  # Remove feature dimension
                        targets = y.float()  # [B, num_nodes]
                        
                        # Compute loss for batch
                        batch_loss = criterion(predict, targets).item()
                        ...
                        val_all_logits.extend(predict.cpu().flatten().tolist())
    ...
    def _plot_training_curves_interim(self, history, current_epoch, seq_length, num_features, 
        ...
        val_epochs = range(5, len(history['train_loss']) + 1, 5) if len(history['val_loss']) > 0 else []
    ...
    def _plot_training_curves(self, history, num_epochs, seq_length, num_features, 
        ...
        if len(history['val_loss']) > 0:
            ax.plot(val_epochs, history['val_loss'], 'r-', label='Val Loss', linewidth=2, marker='s')
        ...
        if len(history['val_acc']) > 0:
            ax.plot(val_epochs, history['val_acc'], 'r-', label='Val Accuracy', linewidth=2, marker='s')
        ax.set_xlabel('Epoch', fontsize=12, fontweight='bold')
    ...
    def test(self, test_dataset, seq_length, num_features, batch_size=32, config=None, threshold=0.5):
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                output = self.model(x)
                output_for_loss = output[:, :, :, -1]       # [B, C, N]
                predict = output_for_loss.squeeze(1)        # remove feature dim
        ...
        y_pred = torch.cat(all_preds, dim=0).numpy()    # [num_samples, num_nodes]
        y_true = torch.cat(all_targets, dim=0).numpy()  # [num_samples, num_nodes]
        ...
        # Return flattened arrays to match validation behavior
        return {'preds': y_pred.flatten(), 'targets': y_true.flatten()}